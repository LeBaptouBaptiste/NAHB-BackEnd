...
exports.storyGenerator = exports.StoryGenerator = void 0;
...
class StoryGenerator {
    ollama;
    knowledge;
    constructor(ollama = ollamaClient_1.ollamaClient, knowledge = knowledgeBase_1.knowledgeBase) {
        this.ollama = ollama;
     ...
     */
    async generateFullStory(request, authorId = 'ai-system') {
        ...
        try {
            ...
            const firstPage = await this.generateFirstPage(story._id.toString(), metadata.title, metadata.description, metadata.theme || 'adventure', language);
            ...
            await this.generateRemainingPages(story._id.toString(), metadata.title, metadata.theme || 'adventure', firstPage, numPages, language);
    ...
    async generatePage(request) {
        ...
        try {
            ...
            let previousContent = '';
            if (previousPageId) {
                const previousPage = await Page_1.Page.findById(previousPageId);
                if (previousPage) {
                    previousContent = previousPage.content;
                }
            }
            // Build prompt
            const prompt = promptTemplates_1.PromptTemplates.generatePagePrompt(storyContext, previousContent, knowledgeContext);
            // Generate
            ...
            return parsedPage; // Return parsed structure
        }
        catch (error) {
     ...
     */
    async generateStoryMetadata(userPrompt, language) {
        const prompt = promptTemplates_1.PromptTemplates.generateMetadataPrompt(userPrompt, language);
        const rawOutput = await this.ollama.generate(prompt, {
            temperature: 0.7,
            num_predict: 300,
        });
     ...
     */
    async generateFirstPage(storyId, title, description, theme, language) {
        const prompt = promptTemplates_1.PromptTemplates.generateFirstPagePrompt(title, description, theme, language);
        const rawOutput = await this.ollama.generate(prompt, {
            temperature: 0.8,
            num_predict: 800,
        });
     ...
     */
    async generateRemainingPages(storyId, title, theme, firstPage, targetPages, language) {
        const queue = [];
        ...
        while (queue.length > 0 && pageCount < maxPages) {
            ...
            pageCount++;
            const shouldBeEnding = (pageCount >= maxPages - 1 || // Last pages should be endings
            ...
            try {
                const prompt = promptTemplates_1.PromptTemplates.generateContinuationPagePrompt({
                    title,
                    theme,
                ...
                await Page_1.Page.updateOne({ _id: pending.parentPageId }, { $set: { [`choices.${pending.choiceIndex}.targetPageId`]: newPage._id.toString() } });
     ...
     */
    async suggestChoices(request) {
        const { pageContent, storyContext, numChoices = 3 } = request;
        try {
            const knowledgeContext = this.knowledge.getGenerationContext();
            const prompt = promptTemplates_1.PromptTemplates.suggestChoicesPrompt(pageContent, storyContext, knowledgeContext, numChoices);
            const rawOutput = await this.ollama.generate(prompt, {
                temperature: 0.7,
                num_predict: 800,
            });
     ...
     */
    async parseGeneratedStory(markdown) {
        try {
            // Parse directly from markdown
            const lines = markdown.split('\n');
            // Extract title (look for # or ## at start)
            const titleLine = lines.find(l => l.match(/^#\s+[^#]/));
            const title = titleLine ? titleLine.replace(/^#+\s+/, '').trim() : 'Untitled Story';
            // Extract description (usually after title)
            const descLine = lines.find(l => l.match(/^##\s+[^#]/) && !l.includes('Page'));
            const description = descLine ? descLine.replace(/^#+\s+/, '').trim() : 'An interactive adventure';
            ...
            for (let i = 0; i < pageMarkers.length; i++) {
                ...
                const isEnding = pageContent.toLowerCase().includes('fin') ||
                    pageContent.toLowerCase().includes('game over') ||
                    choices.length === 0;
                pages.push({
                    content: pageContent.trim(),
                    isEnding,
                    endingType: pageContent.toLowerCase().includes('game over') ? 'death' : undefined,
                    choices
                });
            }
            return {
     ...
     */
    basicParseStory(markdown) {
        const lines = markdown.split('\n');
        const title = lines.find(l => l.startsWith('# '))?.replace('# ', '').trim() || 'Untitled Story';
        const description = lines.find(l => l.startsWith('## ') && !l.includes('Page'))?.replace('## ', '').trim() || 'An interactive adventure';
        return {
            title,
            description,
            theme: 'adventure',
     ...
     */
    extractChoicesFromPage(pageContent) {
        const choices = [];
     ...
     */
    async saveStoryToDatabase(generated, request) {
        ...
        await story.save();
        // Create pages
        const pageMap = new Map(); // pageNumber -> mongoId
        ...
        const firstPage = await Page_1.Page.findOne({ storyId: story._id.toString() });
        if (firstPage) {
            story.startPageId = firstPage._id.toString();
            await story.save();
        }
...
exports.storyGenerator = new StoryGenerator();
