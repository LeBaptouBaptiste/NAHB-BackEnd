...
async function seedMainStory() {
	try {
		...
		if (existingStory) {
			...
			await Page.deleteMany({ storyId: existingStory._id });
			await Story.deleteOne({ _id: existingStory._id });
		}
		...
		const story = await Story.create({
			title: "L'Éveil dans les Ténèbres",
			description:
				"Une aventure interactive avec système de dés et choix de classe. Incarnez un Guerrier, un Mage ou un Assassin dans un donjon rempli de dangers mortels.",
			tags: [
				"fantasy",
				"rpg",
				"interactive",
...
🪨 **Un caillou complètement inutile** (mais brillant, donc précieux)`,
		...
		const page4bis = await Page.create({
			storyId: story._id,
...
**Toi :** *"..."*`,
		...
		// ==================== PAGE 10: Exit Search ====================
		const page10 = await Page.create({
			storyId: story._id,
...
🌬️ **Porte de droite** : Un courant d'air frais... et des grattements`,
		...
		const page11 = await Page.create({
			storyId: story._id,
...
✅ **Le dragon te laisse passer librement !**`,
		...
		const page12 = await Page.create({
			storyId: story._id,
...
<14: 💀 GAME OVER`,
		...
		const page13 = await Page.create({
			storyId: story._id,
...
- Un trésor scintillant derrière lui`,
		...
		const page14 = await Page.create({
			storyId: story._id,
...
<12: 💀 GAME OVER`,
		...
		const page15 = await Page.create({
			storyId: story._id,
...
Tu peux le libérer ou l'attaquer dans sa faiblesse.`,
		...
		const page16 = await Page.create({
			storyId: story._id,
...
**Dragon :** *"Très bien, pose tes questions, mortel. Mais fais vite."*`,
		...
		const page17 = await Page.create({
			storyId: story._id,
...
Lance 1d20 + Bonus Mage (+2)`,
		...
		const page17bis = await Page.create({
			storyId: story._id,
...
Un moment de silence passe entre vous.`,
		...
		const page18 = await Page.create({
			storyId: story._id,
...
💰 Tu aperçois des monceaux d'or, des gemmes, des armes légendaires...`,
		...
		const page19 = await Page.create({
			storyId: story._id,
...
Tu glisses et tombes dans le magma. Mort instantanée.`,
		...
		// ==================== PAGE 20: Chest Room ====================
		const page20 = await Page.create({
			storyId: story._id,
...
Trop beau pour être vrai...`,
		...
		const page20bis = await Page.create({
			storyId: story._id,
...
**AUTRES:** +1`,
		...
		const page21 = await Page.create({
			storyId: story._id,
			content: `🏆 **VICTOIRE !**
...
🎉 **FIN A : "Le Pillard Triomphant"**`,
		...
		const page22 = await Page.create({
			storyId: story._id,
...
🎉 **FIN B : "Le Mage Allié des Dragons"**`,
		...
		const gameOverDragon = await Page.create({
			storyId: story._id,
			content: `💀 **GAME OVER**
...
Fin instantanée. Tu es réduit en cendres.`,
		...
		const gameOverGeneric = await Page.create({
			storyId: story._id,
			content: `💀 **GAME OVER**
...
Ta aventure se termine ici dans les ténèbres du donjon.`,
		...
		const gameOverShadow = await Page.create({
			storyId: story._id,
			content: `💀 **GAME OVER**
...
Tu t'effondres, la vie quittant ton corps.`,
		...
		const gameOverMimic = await Page.create({
			storyId: story._id,
			content: `💀 **GAME OVER**
...
Tes cris résonnent dans la salle vide.`,
		...
		await Page.findByIdAndUpdate(page1._id, {
			choices: [
		...
		await Page.findByIdAndUpdate(page2._id, {
			choices: [
				{ text: "Descendre les escaliers", targetPageId: page3._id.toString() },
				{ text: "Fouiller la cellule", targetPageId: page4._id.toString() },
				{
					text: "Crier pour attirer l'attention",
					targetPageId: page5._id.toString(),
				},
				{
					text: "[ASSASSIN] Chercher un passage secret (1d20 ≥12)",
					targetPageId: page4bis._id.toString(),
				},
				{
					text: "[MAGE] Lancer une boule de feu",
					targetPageId: gameOverGeneric._id.toString(),
				},
				{
					text: "[GUERRIER] Défoncer le mur",
					targetPageId: page2._id.toString(),
				},
			...
			hotspots: [
				{
					...
					diceRoll: {
						...
						difficulty: 12,
						type: "custom",
						failurePageId: page2._id.toString(),
					},
				},
				{
					x: 10,
		...
		await Page.findByIdAndUpdate(page3._id, {
			choices: [
				...
				{
					text: "[GUERRIER] Hurler un cri de guerre",
					targetPageId: gameOverDragon._id.toString(),
				},
				{
					text: "[ASSASSIN] Marcher sur les murs",
					targetPageId: gameOverGeneric._id.toString(),
				},
				{
					text: "[MAGE] Utiliser Lévitation",
					targetPageId: page7._id.toString(),
				},
			],
		});

		// Page 4 → Back to Page 2 with items
		await Page.findByIdAndUpdate(page4._id, {
			choices: [
				...
				{
					text: "[MAGE] Analyser le médaillon",
					targetPageId: page8._id.toString(),
				},
				{
					text: "[GUERRIER] Casser le médaillon",
					targetPageId: gameOverGeneric._id.toString(),
				},
			],
		});
		...
		await Page.findByIdAndUpdate(page4bis._id, {
			choices: [
				{
					text: "Retourner dans la cellule",
					targetPageId: page2._id.toString(),
				},
			],
		});
		...
		await Page.findByIdAndUpdate(page5._id, {
			choices: [
				{ text: "Courir vers l'escalier", targetPageId: page3._id.toString() },
				{
					text: "[COMBAT] Affronter l'ombre (1d20+Combat ≥15)",
					targetPageId: page3._id.toString(),
				},
				{
					text: "[<15] Mort par l'ombre",
					targetPageId: gameOverShadow._id.toString(),
				},
				{
					text: "[FUITE] Fuir (1d20+Fuite ≥12)",
					targetPageId: page2._id.toString(),
				},
		...
		await Page.findByIdAndUpdate(page6._id, {
			choices: [
				{
				...
				{
					text: "Utiliser le médaillon (si possédé)",
					targetPageId: page11._id.toString(),
				},
				{
					text: "[GUERRIER] Crier un cri de guerre",
					targetPageId: gameOverDragon._id.toString(),
				},
				{ text: "[MAGE] Lancer un sort", targetPageId: page12._id.toString() },
				{
					text: "[ASSASSIN] Backstab furtif",
					targetPageId: gameOverDragon._id.toString(),
				},
				{
					text: "Se cacher derrière un pilier",
					targetPageId: page7._id.toString(),
				}, // Side choice
			],
			hotspots: [
		...
		await Page.findByIdAndUpdate(page7._id, {
			choices: [
				{ text: "Continuer d'observer", targetPageId: page13._id.toString() },
				{
					text: "Frapper maintenant",
					targetPageId: gameOverDragon._id.toString(),
				},
				{
					text: "Chercher un point faible",
					targetPageId: page14._id.toString(),
				},
				{
					text: "[ASSASSIN] Analyser ses mouvements (1d20+2 ≥14)",
					targetPageId: page14._id.toString(),
				},
				{ text: "[MAGE] Vision magique", targetPageId: page15._id.toString() },
				{
					text: "[GUERRIER] Retenir son souffle",
					targetPageId: gameOverGeneric._id.toString(),
				},
			],
		});
		...
		await Page.findByIdAndUpdate(page8._id, {
			choices: [
		...
		await Page.findByIdAndUpdate(page9._id, {
			choices: [
				{ text: "Je cherche la sortie", targetPageId: page10._id.toString() },
				{
					text: "Je veux te combattre",
					targetPageId: gameOverDragon._id.toString(),
				},
				{ text: "J'ai des questions", targetPageId: page16._id.toString() },
				{
					text: "[MAGE] Je veux négocier un pacte",
					targetPageId: page17._id.toString(),
				},
				{
					text: "[GUERRIER] Bras de fer ?",
					targetPageId: gameOverDragon._id.toString(),
				},
				{
					text: "[ASSASSIN] Ton trésor est où ?",
					targetPageId: page18._id.toString(),
				},
			],
		});
		...
		// Page 10 → Two doors
		await Page.findByIdAndUpdate(page10._id, {
			choices: [
				{
					text: "Prendre à gauche (lave)",
					targetPageId: page19._id.toString(),
				},
				{
					text: "Prendre à droite (coffre)",
					targetPageId: page20._id.toString(),
				},
		...
		await Page.findByIdAndUpdate(page11._id, {
			choices: [
		...
		// Page 12 → Magic duel
		await Page.findByIdAndUpdate(page12._id, {
			choices: [
				{
					text: "[COMBAT] Lancer un sort de glace (1d20+Combat ≥18)",
					targetPageId: page21._id.toString(),
				},
				{
					text: "[≥18] Victoire héroïque !",
					targetPageId: page21._id.toString(),
				},
		...
		await Page.findByIdAndUpdate(page13._id, {
			choices: [
				{
					text: "[FUITE] Voler du trésor (1d20+Fuite ≥16)",
					targetPageId: page21._id.toString(),
				},
				{ text: "[≥16] Succès !", targetPageId: page21._id.toString() },
				{
					text: "[<16] Pris en flag",
					targetPageId: gameOverDragon._id.toString(),
				},
		...
		await Page.findByIdAndUpdate(page15._id, {
			choices: [
				{
					text: "Briser la chaîne spectrale",
					targetPageId: page22._id.toString(),
				},
				{
					text: "Attaquer le dragon affaibli",
					targetPageId: page14._id.toString(),
				},
			],
		});
		...
		// Page 16 → Questions
		await Page.findByIdAndUpdate(page16._id, {
			choices: [
				{ text: "Comment sortir d'ici ?", targetPageId: page10._id.toString() },
				{
					text: "Pourquoi gardes-tu ce trésor ?",
					targetPageId: page17bis._id.toString(),
				},
				{
					text: "Qui es-tu vraiment ?",
					targetPageId: page17bis._id.toString(),
				},
			],
		});
		...
		await Page.findByIdAndUpdate(page17._id, {
			choices: [
				{
					text: "Proposer un pacte (1d20+Persuasion ≥14)",
					targetPageId: page22._id.toString(),
				},
				{
					text: "[≥14] Le dragon accepte",
					targetPageId: page22._id.toString(),
				},
		...
		await Page.findByIdAndUpdate(page17bis._id, {
			choices: [
				{
		...
		await Page.findByIdAndUpdate(page18._id, {
			choices: [
				{ text: "Tenter de voler", targetPageId: page13._id.toString() },
				{
					text: "Négocier un partage (1d20 ≥15)",
					targetPageId: page21._id.toString(),
				},
		...
		await Page.findByIdAndUpdate(page20._id, {
			choices: [
			...
			hotspots: [
				{
					...
					diceRoll: {
						...
						difficulty: 13,
						type: "custom",
						failurePageId: page20bis._id.toString(),
					},
				},
			],
		});
		...
		await Page.findByIdAndUpdate(page20bis._id, {
			choices: [
				{
					text: "[COMBAT] Combattre le Mimic (1d20+Combat ≥14)",
					targetPageId: page21._id.toString(),
				},
				...
				{
					text: "[FUITE] Fuir le Mimic (1d20+Fuite ≥10)",
					targetPageId: page10._id.toString(),
				},
