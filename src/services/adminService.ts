...
export const toggleStorySuspension = async (storyId: string, suspended: boolean) => {
    ...
    return { message: suspended ? 'Story suspended' : 'Story unsuspended', story };
};

export const getAllReports = async (status?: string, page: number = 1, limit: number = 20) => {
    const filter: any = {};
    if (status) {
...
export const updateReportStatus = async (adminId: number, reportId: string, status: string, adminNotes?: string) => {
...
export const getPlatformStats = async () => {
    ...
    return {
        ...
        ratings: {
            average: globalRatingStats ? Math.round(Number(globalRatingStats.averageRating) * 10) / 10 : 0,
            total: globalRatingStats ? Number(globalRatingStats.totalRatings) : 0,
        },
    };
};

export const getAllStories = async (status?: string, page: number = 1, limit: number = 20, search?: string) => {
    const filter: any = {};
    if (status) {
        filter.status = status;
    }
    ...
    const stories = await storyRepository.find(filter, { createdAt: -1 }, skip, limit);
