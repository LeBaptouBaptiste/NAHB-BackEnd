...
export const startGame = async (userId: number, storyId: string, preview: boolean) => {
...
export const makeChoice = async (userId: number, sessionId: string, choiceIndex: number, hotspotIndex: number, diceRollSuccess?: boolean) => {
    ...
    if (hotspotIndex !== undefined) {
        ...
        if (choiceIndex === undefined || choiceIndex < 0 || choiceIndex >= page.choices.length) {
...
export const getPathStats = async (userId: number, sessionId: string) => {
    ...
    const allSessions = await gameRepository.findCompletedByStory(session.storyId);
    ...
    const currentEndingPage = await pageRepository.findById(session.currentPageId);
