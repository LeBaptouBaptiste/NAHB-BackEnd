...
export class AIStoryController {
    ...
    async generatePage(req: Request, res: Response): Promise<void> {
        try {
            const { storyContext, previousPageId, theme } = req.body as PageGenerationRequest;
    ...
    async suggestChoices(req: Request, res: Response): Promise<void> {
        try {
            const { pageContent, storyContext, numChoices } = req.body as ChoiceSuggestionRequest;
