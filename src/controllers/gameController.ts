...
export const startGame = async (req: Request, res: Response) => {
	...
	try {
		...
		if (err.message === "Story has no start page")
			return res.status(400).json({ message: err.message });
		if (err.message === "Only the author can preview this story")
			return res.status(403).json({ message: err.message });
		if (err.message === "Story is not published")
			return res.status(400).json({ message: err.message });
...
export const makeChoice = async (req: Request, res: Response) => {
	...
	try {
		const session = await gameService.makeChoice(
			userId,
			sessionId,
			choiceIndex,
			hotspotIndex,
		...
		if (err.message === "Current page not found")
			return res.status(404).json({ message: err.message });
		if (
			err.message === "Invalid hotspot index" ||
			err.message === "Invalid choice index" ||
			err.message === "Target page not available"
		) {
			return res.status(400).json({ message: err.message });
		}
...
export const getSessionByStory = async (req: Request, res: Response) => {
	...
	try {
		...
		const session = await gameService.getSessionByStory(
			userId.toString(),
			storyId
		);
